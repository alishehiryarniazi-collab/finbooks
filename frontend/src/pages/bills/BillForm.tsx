...
import type { Account, Vendor, TaxRate } from "../../lib/types";
...
export function BillForm() {
  ...
  // In edit mode, load the draft bill and prefill the form.
  useEffect(() => {
    if (!isEdit) return;
    let ignore = false;
    (async () => {
  ...
  if (vendorsReq.loading || accountsReq.loading || taxRatesReq.loading || loadingDoc)
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const { data } = isEdit
        ? await api.patch<{ bill: { id: string } }>(`/bills/${id}`, payload)
        : await api.post<{ bill: { id: string } }>("/bills", payload);
  ...
  return (
    <div>
      <PageHeader
        title={isEdit ? t("forms.editBill") : t("forms.newBill")}
      ...
      <Card>
        <form onSubmit={submit} className="flex flex-col gap-5">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <TextField
              label={t("forms.billNumberOptional")}
              value={number}
              onChange={(e) => setNumber(e.target.value)}
              placeholder={t("forms.autoBill")}
            />
            <TextField
              label={t("fields.billDate")}
              type="date"
              value={billDate}
            ...
            <table className="w-full min-w-[720px] text-sm">
              <thead>
                <tr className="text-left text-slate-400">
                  ...
                  <th className="px-2 py-1 font-medium">{t("fields.expenseAccount")}</th>
