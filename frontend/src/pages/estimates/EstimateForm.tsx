...
export function EstimateForm() {
  ...
  const [expiryDate, setExpiryDate] = useState(() => inputDate(new Date(Date.now() + 30 * 864e5)));
  const [lines, setLines] = useState<Line[]>([emptyLine()]);
  const [error, setError] = useState<string | null>(null);
  const [busy, setBusy] = useState(false);
  const [loadingDoc, setLoadingDoc] = useState(isEdit);

  // In edit mode, load the estimate and prefill the form.
  useEffect(() => {
    if (!isEdit) return;
    let ignore = false;
    (async () => {
      try {
        ...
        setIssueDate(inputDate(est.issueDate));
        setExpiryDate(inputDate(est.expiryDate));
        setLines(
          est.lines.map((l) => ({
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const { data } = isEdit
        ? await api.patch<{ estimate: { id: string } }>(`/estimates/${id}`, payload)
        : await api.post<{ estimate: { id: string } }>("/estimates", payload);
  ...
  return (
    <div>
      <PageHeader
        title={isEdit ? t("forms.editEstimate") : t("forms.newEstimate")}
        subtitle={t("forms.estimateSubtitle")}
      />
      <Card>
        <form onSubmit={submit} className="flex flex-col gap-5">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <TextField
              label={t("forms.estimateNumberOptional")}
              value={number}
              onChange={(e) => setNumber(e.target.value)}
              placeholder={t("forms.autoEstimate")}
            />
            <TextField
